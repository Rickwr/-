...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 表单元素修改商品个数
      changeSkuNum(event){
        // 用户输入的值*1，值为非法文本则为NAN
        let value=event.target.value*1;
        // 如果用户输入的文本为非法：出现NAN或者小于1
      ...
      async updateCart(){
        // 1.发请求，将商品更新到服务器上
        try {
            await this.$store.dispatch('detail/reqUpdateCartList',{skuId:this.$route.params.skuId,skuNum:this.skuNum});
        // 2.更新服务器成功，路由跳转
        this.$router.push({name:'addcartsuccess',query:{skuNum:this.skuNum}});
        // 路由跳转时还需要将商品信息带给兄弟路由
        // 简单的数据可以通过query传递
        // 复杂的数据应通过会话存储传递（不持久化：会话结束数据消失）
        // 存储字符串所以需要使用stringify
